...
We have identified the key stakeholders starting with the person who triggers the value stream aka the customer as well as the key actors involved in getting the pizza order out the door

![[Pizzeria Order Preparation Value Stream - Motivation View.png]]
The value stream motivation perspective provides the first part of the context for the analysis that is being asked for. If only business was that simple. In reality, most organizations are part of a value stream network where there are dependencies that need to be understood to be able to perform the analysis of the data.

For Stealth EA Pizzeria, let's look at a simple value stream network in action.