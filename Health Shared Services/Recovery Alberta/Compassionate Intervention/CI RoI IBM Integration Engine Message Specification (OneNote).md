...
> [!note] OneNote-safe version
> Reformatted copy of CI RoI IBM Integration Engine Message Specification for pasting into OneNote. Code blocks and diagrams are indented with non-breaking spaces so alignment survives the paste — do not paste JSON from this file into a parser; use the original spec for that.
>
> To paste: copy from Obsidian **reading view**, paste into OneNote, then select the code/diagram blocks and set a monospace font (Consolas) so columns line up.
...
Per the CIP-to-IBM sequence, the Snowflake query occurs at step 3a of the automated path — after a successful `CreateRelease2` call returns a `ReleaseID` from Epic, and before the webhook delivery to CIP. The Akana API Gateway is the primary connection point between CIP and HSS.
