...
- **Last Update**
  ...
  - **June 25, 2026 (Alec Blair)** — Synced to [[OMRS Database ERD]] v2.5: the Export List action now writes a `DisclosureExportLog` record (HIA PHI-disclosure audit — scope, row count, disease group, purpose, privacy-boundary flag), since `AuditLog` captures only record mutations. Updated FB 8 and the Security Specifications note.
