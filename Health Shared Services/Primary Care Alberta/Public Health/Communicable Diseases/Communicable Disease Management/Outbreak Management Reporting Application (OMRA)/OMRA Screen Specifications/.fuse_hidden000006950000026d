...
description: Build specification for the OMRS Communicable Disease Search screen — a unified search-and-triage entry point that, in its first build, lets an investigator find, filter and open outbreak investigations, and is designed to extend to contact and CD-episode search.
tags:
...
It depends on the Foundation maintenance screens that supply its lookups — [[User Maintenance Screen Specifications|User Maintenance]] (the RBAC/ABAC access model that scopes results), Location / Facility Maintenance, and [[Communicable Disease Maintenance Screen Specifications|Communicable Disease Maintenance]] (the disease/organism dictionary). Connect Care remains the **system of record for individual cases** (see [[CLAUDE-OMRS]] data-ownership boundaries); OMRS owns the outbreak coordination record, so the result grid is a window onto the `Outbreak` entity and its references in [[OMRS Database ERD]].
...
1. **Scenario Conditions** — What scenarios have been defined that may affect the specifications.
   ...
   1. *User group affiliation.* Should the search default to the investigations for the group the logged-in user is assigned to — e.g., CDC sees CDC — while users assigned to multiple departments see more? This maps to the OMRS ABAC scoping attributes (`UserTeam`, `UserZone`, `UserDiseaseGroup` in [[OMRS Database ERD]] Section 14). **Open question:** confirm whether the default *filter* is the user's group affiliation, separate from the *privacy* scoping (HIV/STI disease-group boundaries) that is always enforced regardless of role.
...
3. **User Experience Considerations** — What (if any) UX options might we have to consider as part of the build.
   ...
   1. Keep the screen visually consistent with the other OMRS investigation screens ([[Create Outbreak Investigation Screen Specifications]], [[Contact Identification Screen Specifications]]) — shared filter-panel and result-grid patterns.
...
- **Access Specifications** — The source states **all roles have access** to search; what remains to confirm is whether searching is restricted at all, and whether results **default** to the user's group affiliation (CDC sees CDC; multi-department users see more). Regardless of the default, result *visibility* is scoped by the OMRS two-layer access model — functional role plus team / zone / disease-group attributes ([[User Maintenance Screen Specifications]]) — and **disease-group privacy boundaries (HIV/STI) are always enforced**.
...
- **Last Update**
  ...
  - **June 25, 2026 (Alec Blair)** — Brought the stub up to the OMRS screen-specification standard used by [[Create Outbreak Investigation Screen Specifications]] and [[Communicable Disease Maintenance Screen Specifications]]: added OKF frontmatter; wrote the Design Specification Context and Wireframe sections; populated the seven Acceptance Criteria sections from the imported *Outbreak Search* functional draft and the [[Outbreak Search - Outbreak Investigator User Story]]; and built the field-mapping table against [[OMRS Database ERD]] v2.5. Framed the screen as a unified Communicable Disease Search with **outbreak search as the first build** and contact / CD-episode search staged behind it. Flagged ERD gaps for resolution before build — no `Outbreak` region/`zoneID` column, no direct `Outbreak.outbreakSettingID`, no distinct closed-date column, and no store for inbound Epic deep-link parameters (Encounter ID / PHN / ULI) — and carried open questions on group-affiliation default scoping and region-level definitions.
