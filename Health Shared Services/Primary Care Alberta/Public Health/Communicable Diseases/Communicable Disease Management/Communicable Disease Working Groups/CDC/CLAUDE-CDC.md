...
- [[Create Exposure Incident ID - CDC Investigator User Story]] (**O-C-5**, _current state_) — CDC Investigator requests generation of an Exposure Incident (EI) ID (ProvLab EI number / zone-specific OI number) so non-patient-specific lab requests can be tracked and results associated to an outbreak. Captures the lab agreement that the OMRS **Outbreak ID** is accepted as the lab-sample identifier for human, food, animal, and environmental samples. **Current-state record, pending the Epic-migration review of Outbreak Identifier creation** (CD-OI-3); reassigned from the source O-A-4 to avoid collision with the Create Outbreak story.
...
_Version_: 1.4 (added the O-A-6 Assessment story and the O-C-5 current-state Create Exposure Incident ID story to the story list)
