...
1. **Capture the call log in the call-centre tooling.** *Given* an inbound facility outbreak call, *when* the analyst handles it, *then* the call is logged using **Genesys** and **Epic Cheers** (CRM), confirming the call-capture system of record for the encounter. [Confirm Genesys / Epic Cheers integration boundary with OMRS — see Dependencies]
...
8. **Privacy-conforming documentation and audit.** *Given* the create/edit and share actions on the encounter, *when* they are performed, *then* they are scoped to the analyst's authorized team / zone per the OMRS access model and audited (actor, timestamp, before/after). The encounter may carry personal and personal-health information collected during the call. [Health Information Act (HIA), RSA 2000, c H-5 — collection, use and disclosure, [verify section]]; [[User Maintenance Screen Specifications]]
...
3. **Pre-populate the referral from the encounter.** A referral disposition pre-populates the downstream [[Create Outbreak Investigation - CDC Investigator User Story|Create Outbreak]] / [[Outbreak Assessment - CDC Investigator User Story|Assessment]] record from the encounter, so the investigator does not re-key the intake details.
...
- **Referral mechanism from Genesys / Epic Cheers (CD-OI-14).** How the Genesys / Epic Cheers call log and any attached line list transition into OMRS and the SHE / CDC investigator teams (manual hand-off, interface, or shared record), and where the structured referral lives, is an open design decision tracked as **CD-OI-14** in [[Communicable Disease Open Issues]]. This determines whether AC2–AC7 are captured in the CRM, in OMRS, or split.
...
- **Downstream investigator stories (O-A-4 / O-A-6).** The referral produced here feeds [[Create Outbreak Investigation - CDC Investigator User Story|Create Outbreak (O-A-4)]] and [[Outbreak Assessment - CDC Investigator User Story|Assessment (O-A-6)]]; confirm the referral payload matches what those stories expect to consume.
...
- **Encounter / referral ERD entity.** Confirm whether a PPHST encounter / referral entity exists in the [[OMRS Database ERD]] or must be added to carry the intake metadata, teaching status, disposition, and line-list attachment.
...
| Related Pattern Story        | [[Outbreak Assessment - CDC Investigator User Story]]; [[Submit Facility Outbreak Line List - Facility Operator User Story]]; [[Create Outbreak Investigation - CDC Investigator User Story]] |
| Reference Material           | [[CLAUDE-OMRS]]; [[CLAUDE-CDC]]; [[User Maintenance Screen Specifications]]; setting-specific Outbreak Prevention & Control guides              |
