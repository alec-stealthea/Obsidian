...
2. **Link to a parent (cluster) outbreak and reference non-Alberta outbreak IDs.** *Given* the create flow, *when* the outbreak is part of a multi-site event or referred from another jurisdiction, *then* the investigator can link it to a **parent / cluster outbreak** (`Outbreak.clusterOutbreak`) and record an external identifier — PHAC # (`Outbreak.outbreakPHACID`) and/or an external-agency ID (`ExternalReportSubmission.reportingAgencyID`). [[OMRS Database ERD]] Section 1, Section 9
...
7. **Privacy-conforming creation and audit.** *Given* the create/edit actions, *when* they are performed, *then* they are scoped to the investigator's authorized team / zone / disease group per the OMRS access model and audited via `AuditLog` (actor, timestamp, before/after). The `Outbreak` record is coordination metadata, but it is the parent of records holding personal health information. [[User Maintenance Screen Specifications]]; [Health Information Act (HIA), RSA 2000, c H-5 — collection, use and disclosure, [verify section]]
...
4. **Provision the Outbreak ID as the EI / lab-requisition number.** The Outbreak ID can be provisioned to serve as the Exposure Investigation / lab-requisition identifier so requisitions can be raised before individual patients are known (deferred, [[OMRS Database ERD]] note 10).
...
- **`Outbreak` ERD gaps (later analysis round).** Disease, Setting, Zone/Region and outbreak-level Department do not yet have direct columns on `Outbreak` (no `infectiousDiseaseID`, `outbreakSettingID`, `zoneID`/region, or department FK) — flagged in [[Create Outbreak Investigation Screen Specifications]] and [[OMRS Database ERD]]. To be resolved in a later Create Outbreak / ERD analysis round.
...
| Related Pattern Story        | [[Outbreak Search - Outbreak Investigator User Story]]; [[TB Contact List - TB Nurse User Story]]; [[STI Large Exposure User Story]]            |
| Reference Material           | [[CLAUDE-OMRS]]; [[Contact Identification Screen Specifications]]; [[User Maintenance Screen Specifications]]                                    |
...
- 2026-06-26: Alec Blair — migrated from OneNote into the HSS user-story standard. Added Background, five concrete scenarios (identifiable-patient line list, aggregate reporting, clustered, multi-organism, stand-up-team), two-tier Given/When/Then acceptance criteria with Public Health Act / HIA / AORF traceability, and a Dependencies section. Mapped source fields to the OMRS ERD `Outbreak` entity and related tables (`clusterOutbreak`, `outbreakPHACID`, `OutbreakTeamMember`, `OutbreakOrganism`, `OutbreakDefinition`, `outbreakStatus`/`outbreakProgress`/`outbreakLifecycleStatus`, `AORFSubmission`), linked the [[Create Outbreak Investigation Screen Specifications]] and [[OMRS Database ERD]], and flagged the O-A-2, CD Episode, and O-ET-1/O-M-1 dependencies for linking once those notes exist.
