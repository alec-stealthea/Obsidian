...
The line list is the identifiable-patient feed referenced by the [[02 - Create Outbreak Investigation - CDC Investigator User Story|Create Outbreak Investigation story]] (O-A-4, AC3) and received during the [[05 - Outbreak Assessment - CDC Investigator User Story|Outbreak Assessment story]] (O-A-6). Its build realisation maps to the **Line List & Case Management** entities in the [[OMRA Database ERD]] Section 5 (`FacilityLineList`, `LineListPerson`, `LineListPersonType`, `CaseClassification`, `LineListPersonSymptom`, with `Room` for in-facility location and `ClientRegistryTransaction` for PHN/ULI validation). Connect Care remains the **system of record for individual cases**; OMRA owns the outbreak line-list submission and coordination record (see [[CLAUDE-OMRA]] data-ownership boundaries). This story sits at the **Exposure Trace (ET)** stage of the Outbreak Value Stream and is the Facility Operator counterpart to the CDC Investigator outbreak-creation work.
...
- **Scenario H — Non-facility outbreak (no facility).** A community, environmental, or genomically linked (WGS) outbreak has no reporting facility. The Investigator builds and completes the same line-list instrument with **no facility attached** — a free-text location/area in place of facility name/address — through the [[Create Facility Questionnaire Screen Specifications|Facility Questionnaire]] non-facility scenario. *(Per the 2026-06-29 decision, this replaces a separate non-facility line-list story — see CD-OI-26/27 in [[Communicable Disease Open Issues]].)*
...
7. **Supply facility name and address — except for a non-facility outbreak.** *Given* a submission, *when* it is created, *then* the facility name and address are captured / confirmed (`Facility.facilityName`, `facilityAddress`); **for a non-facility outbreak (community / environmental / WGS-linked) the facility is not mandatory** and a free-text location/area is captured instead (`FacilityLineList.facilityID` nullable). [[OMRA Database ERD]] Section 4; [[Create Facility Questionnaire Screen Specifications]] (Scenario 1.4 / BR 5.5)
...
| Related Pattern Story        | [[02 - Create Outbreak Investigation - CDC Investigator User Story]]; [[TB Contact List - TB Nurse User Story]]; [[STI Large Exposure User Story]] |
| Reference Material           | [[CLAUDE-OMRA]]; [[05 - Outbreak Assessment - CDC Investigator User Story]]; [[Create Exposure Incident ID - CDC Investigator User Story]] |
...
- 2026-06-29: Alec Blair — added **Scenario H (non-facility outbreak)** and amended **AC7** so facility name/address is **not mandatory** for a community/environmental/WGS-linked outbreak (free-text location, `FacilityLineList.facilityID` nullable). Per the 2026-06-29 decision (CD-OI-26 resolved), the non-facility line list is handled as a scenario of the [[Create Facility Questionnaire Screen Specifications|Facility Questionnaire]] rather than a separate story; build consequence tracked as CD-OI-27.
