...
Individual cases of a communicable disease are worked as **CD Episodes** in Connect Care (Epic) — each client's episode carries a **Communicable Disease Abstract** that is the clinical record of that case (see [[Interface Specification - Create CD Episode and Draft Contact Abstract (OMRS to Connect Care)|Create CD Episode and Draft Contact Abstract]]). Per the [[CLAUDE-OMRS]] data-ownership boundaries, Connect Care is the **system of record for individual cases**, while the Outbreak Management Reporting System (OMRS) owns the **outbreak coordination record** and provisions the **Outbreak ID** when an outbreak is created ([[Create Outbreak Investigation - CDC Investigator User Story|Create Outbreak]], O-A-4, AC1).
...
4. **Outbreak ID resolves to a real, declared outbreak.** *Given* an Outbreak ID being applied, *when* it is recorded on an abstract, *then* it must match an existing, declared `Outbreak` record in OMRS (referential integrity), so an abstract cannot be tagged with an invalid or undeclared outbreak.
...
6. **Privacy-conforming update and audit.** *Given* the update action, *when* it is performed, *then* it is scoped to the investigator's authorized team / zone / disease group per the OMRS / Connect Care access model and audited (actor, timestamp, before/after Outbreak ID value), since the abstract carries personal health information. [[User Maintenance Screen Specifications]]; [Health Information Act (HIA), RSA 2000, c H-5 — collection, use, disclosure and audit, [verify section]]
...
5. **Reconciliation between OMRS and Connect Care.** A periodic check confirms that the set of abstracts carrying a given Outbreak ID in Connect Care matches the outbreak's case set / line list in OMRS, flagging discrepancies for the investigator.

## Dependencies

- **Automation feasibility (the governing question).** Whether the Outbreak ID back-fill can be automated (Desirable AC1/AC2) depends on Connect Care (Epic) build capability and the OMRS↔Connect Care integration pattern. Confirm with the Epic Connect Care and Outbreak Application teams before committing the automated criteria; until then the manual MVP stands.
- **Reporting surface.** The "clients meeting the case definition without an Outbreak ID" report (AC2) is expected to be an **Epic report / work-queue**; confirm ownership, the report platform (Connect Care reporting vs. OMRS), and the case-definition logic that drives membership.
- **Outbreak ID provenance.** The Outbreak ID is provisioned by [[Create Outbreak Investigation - CDC Investigator User Story|Create Outbreak]] (O-A-4, AC1) in OMRS; this story consumes it. Any change to how the Outbreak ID is generated (see the Epic-migration review noted in [[Create Exposure Incident ID - CDC Investigator User Story|Exposure Incident ID]] and [[Communicable Disease Open Issues#Design decisions & data-model gaps|CD-OI-3]]) affects what gets stamped here.
- **Where the Outbreak ID lives on the abstract.** Confirm the field on the Connect Care CD Abstract that holds the Outbreak ID and its cardinality (one Outbreak ID per abstract vs. multiple for clustered outbreaks — Scenario E), and reflect it in the abstract / interface design ([[Interface Specification - Create CD Episode and Draft Contact Abstract (OMRS to Connect Care)]]).
...
| Related Design Spec          | [[Interface Specification - Create CD Episode and Draft Contact Abstract (OMRS to Connect Care)]]; [[OMRS Database ERD]] |
...
| Reference Material           | [[CLAUDE-OMRS]]; [[CLAUDE-CDC]]; [[User Maintenance Screen Specifications]]; [[Communicable Disease Open Issues]]      |
