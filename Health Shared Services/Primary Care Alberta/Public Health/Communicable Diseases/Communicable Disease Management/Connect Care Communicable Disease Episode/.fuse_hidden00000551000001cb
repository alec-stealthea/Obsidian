...
# Interface Specification — Create CD Episode and Draft Contact Abstract (OMRS → Connect Care)

> **Direction:** OMRS → Connect Care (Epic) &nbsp;|&nbsp; **Pattern:** Investigator-triggered, with ULI validation gate and error queue &nbsp;|&nbsp; **Status:** Draft for working-group review

This is the second of the two information exchanges connecting the Contact Identification Investigation to Connect Care. The first — [[Interface Specification - Open Contact Identification from CD Abstract (Connect Care to OMRS)|Open Contact Identification from CD Abstract (Connect Care → OMRS)]] — establishes the investigation from the source case abstract. This exchange closes the loop: once a contact has a **validated Provincial Health Number**, OMRS asks Connect Care to open a **Compass Rose Episode** for that contact and, where possible, to auto-create a **draft Communicable Disease Abstract** pre-populated and linked back to both the OMRS Contact Identification case number and the source case Abstract ID.

> **Naming note.** Per [[CLAUDE-OMRS]], *OMRA* / *AOMS* / *OMRS* are the same system; **OMRS** is used throughout.
...
| Attribute | Value |
|---|---|
| **Source system** | OMRS — Create Contact Identification Investigation List (per-contact Create Episode action) |
...
| **Direction** | Outbound from OMRS (inbound to Connect Care) |
...
| **Outcome** | A Compass Rose Episode is created for the contact; a draft CD Abstract is created and linked (target state); Episode ID (and Abstract ID) returned to OMRS |
...
- The **source case Abstract ID** is known from the investigation header (carried in by the [[Interface Specification - Open Contact Identification from CD Abstract (Connect Care to OMRS)|first exchange]]), so the draft abstract can be linked back to the exposing case.
...
- **OMRS Contact Identification case number** — `ContactIdentification.contactIdentificationID`, written onto the draft abstract so the episode points back to the originating contact record.
- **Source case Abstract ID** — the exposing person's abstract identifier (the OMRS *Source Case ID* / `indexCaseAbstractID`), written onto the contact's draft abstract to realize abstract-to-abstract exposure linking.
...
- **No duplicate promotion.** A contact already at *CD Episode Created* is not re-promoted; duplicates are handled as *Closed – Duplicate* in OMRS rather than creating a parallel episode.
...
- **ULI / identity mismatch on the Connect Care side** → the message is routed to a monitored **error queue** rather than silently creating a mismatched or duplicate patient; the OMRS contact stays unpromoted (Investigation Status unchanged) until reconciled. This error queue is explicitly anticipated in the [[Communicable Disease Solution Architecture|Inbound Create CD Episode interface]].
- **Create Episode interface failure / timeout** → do not lose the contact; surface the error in OMRS, leave Investigation Status unchanged, and allow retry without creating a duplicate episode ([[Contact Identification Screen Specifications]], §6).
- **Episode created but draft Abstract creation fails** → the "create episode" and "create draft abstract" outcomes should be separable so a successful episode is not rolled back; OMRS records the Episode ID and flags the abstract as pending, allowing the abstract to be created or completed manually in Connect Care.
- **Patient not positively identified** → the architecture allows using the Epic MRN to create a patient ID pending final positive identification; confirm whether OMRS should mark such episodes provisional until identification is confirmed.
...
This exchange completes the lineage that the [[Interface Specification - Open Contact Identification from CD Abstract (Connect Care to OMRS)|first exchange]] began. The source case Abstract ID seeded into the OMRS investigation header is written back onto the contact's draft abstract, and the OMRS Contact Identification case number ties the new episode to its originating contact record. On the OMRS side, the promotion is recorded across `ContactIdentification` (`epicEpisodeID`, `promotedAbstractID`), `ContactInvestigationLifecycle` (status transition carrying the `EpicAbstract` link), and `AuditLog`. The result is an end-to-end, auditable chain — source case abstract → contact identification → contact episode/abstract — with the clinical record (episode, rule-out evidence) held in Connect Care as the system of record, and the contact list and investigation lifecycle held in OMRS.
...
- **Episode type selection** — confirm the mapping from OMRS disease group to the specific Compass Rose episode type and abstract variant for CDC, SHE, TB and STI.
...
- [[OMRS Database ERD]] — `ContactIdentification` and `ContactInvestigationLifecycle` (Section 15), `EpicAbstract` (Section 11), `ClientRegistryTransaction` (Section 6).
...
- [[CLAUDE-OMRS]] — data-ownership boundaries; Client Registry (EMPI) used for PHN/ULI validation.

## 11. Version History

- **Last Update**
  - **June 23, 2026 (Alec Blair / Enterprise Architecture, Health Shared Services)** — Initial architecture-level draft of the OMRS → Connect Care exchange that, on a validated PHN, creates a Compass Rose Episode and auto-populated draft contact abstract linked to the Contact Identification case number and the source case Abstract ID. Drawn from the [[Communicable Disease Solution Architecture]] inbound/outbound interface set, the [[SBAR - Contact CD Episode Creation and Outbreak-Cluster Association|episode-creation SBAR]], the [[Contact Identification Screen Specifications]], and the Compass Rose setup guide in this folder.
...
- **Companion Interface** — [[Interface Specification - Open Contact Identification from CD Abstract (Connect Care to OMRS)]]

---

_Source attribution: inbound/outbound interface definitions and abstract-linking concept from [[Communicable Disease Solution Architecture]]; promotion behaviour, ULI/error-queue handling, and field mappings from [[Contact Identification Screen Specifications]]; episode-per-contact decision and validation-before-episode rule from [[SBAR - Contact CD Episode Creation and Outbreak-Cluster Association]]; data model from [[OMRS Database ERD]] (Sections 6, 11, 15); Compass Rose episode/Target/Task setup from `Compass Rose Tasks Setup and Support Guide.pdf` (Epic, May 21 2026) in this folder. Governed by the Health Information Act (HIA) and the Public Health Act._
