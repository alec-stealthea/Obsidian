...
| CD-OI-3 | OMRS | **Outbreak ID as the EI number.** Whether/when the system-generated `Outbreak.outbreakID` becomes the **Exposure Investigation (EI) / lab-requisition** identifier (so requisitions can be raised before patients are known). Deferred decision (ERD note 10). | Open | [[OMRS Database ERD]]; [[Create Outbreak Investigation Screen Specifications]] |
