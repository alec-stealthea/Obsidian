...
> **Scope — facility questionnaires only.** This screen covers the **facility** outbreak questionnaire (the line-list or aggregate instrument a facility completes and returns). **Individual / per-case questionnaires are entered into the Epic Communicable Disease episode**, consistent with the OMRA data-ownership boundary that **Connect Care (Epic) is the system of record for individual cases** (see [[CLAUDE-OMRA]]). OMRA owns the facility-level outbreak instrument and its submissions; once a facility-reported case requires an individual CD episode, the per-case questionnaire detail is captured in Epic, not here. The contact-investigation equivalent (slide 16) is specified separately in [[Contact Identification Screen Specifications]].
...
- **Source user stories** — [[Submit Facility Outbreak Line List - Facility Operator User Story]] (O-ET-1, the line-list instrument), [[Submit Aggregate Outbreak Report - Facility Operator User Story]] (O-M-1, the aggregate instrument), and [[Document Facility Outbreak Phone Encounter - PPHST Call Analyst User Story]] (the PPHST intake that may attach a completed instrument). Anchored by [[Create Outbreak Investigation - CDC Investigator User Story]] (O-A-4) and [[Create Outbreak Investigation Screen Specifications]], from which this screen is launched.
...
2. **Functional Behaviour** — What (if any) business logic needs to be created for this.
   ...
   1. **Pre-definition of the core questionnaire** from what the outbreak already knows. On launch, the builder proposes a default column set derived from:
      ...
      - **Symptom list** — `Symptom` via `DiseaseSymptom` for the disease, seeding the **Signs / Symptoms** options (the [[Submit Facility Outbreak Line List - Facility Operator User Story|O-ET-1]] AC3 requirement to display the outbreak case definition's specific symptoms).
...
4. **Data Inputs and Outputs** — What are the data elements involved for the build object. What test data is needed to support functional testing.
   ...
   2. *Data outputs.* **Build Questionnaire** can produce:
      - **Create Spreadsheet** — an Excel line-list template the facility uploads through the portal, e-mails, or faxes ([[Submit Facility Outbreak Line List - Facility Operator User Story|O-ET-1]] Desirable AC2, the schema-aligned template);
      - **Create PDF** — a fillable/printable PDF for line-list reporting, potentially generated from the Excel form;
      - **Create Web Page** — an Outbreak Incident page supporting daily updates to provided information and the addition of new patients ([[Submit Aggregate Outbreak Report - Facility Operator User Story|O-M-1]] AC8, the accessible web portal).
...
| Location | Lookup | `Outbreak.outbreakFacility` → `Facility.facilityName` (+ `facilityAddress`, `facilityPhone`) | Location/Facility list | Inherited | No | Read-only; facility name/address/phone carried for the output header. Facility **contact person** is itself an ERD gap (see [[Submit Aggregate Outbreak Report - Facility Operator User Story|O-M-1]]). |
...
- **Linked Enabling Stories** — [[Submit Facility Outbreak Line List - Facility Operator User Story]] (O-ET-1); [[Submit Aggregate Outbreak Report - Facility Operator User Story]] (O-M-1); [[Document Facility Outbreak Phone Encounter - PPHST Call Analyst User Story]] (O-C-[TBD])
