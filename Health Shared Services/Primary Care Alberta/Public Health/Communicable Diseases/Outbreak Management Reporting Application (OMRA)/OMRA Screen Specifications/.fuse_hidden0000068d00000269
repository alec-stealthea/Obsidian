...
description: Build specification for the OMRS User Maintenance screen, mapping the OMRS User Maintenance wireframe to the OMRS data model and defining the two-layer (RBAC + ABAC) access model that governs every other screen.
tags:
...
This screen provisions and maintains the **application users** of the Outbreak Management Reporting System (OMRS) and assigns the access each user holds. It is the operational front end for the role-based access scheme described in the [[Communicable Disease Solution Architecture]]: a **Team Lead or Department Manager** looks up a person, assigns them a single **functional role** (what they may do), and attaches the **department** scoping attribute (which outbreaks they may edit) plus a professional designation (how they are identified). Because every other OMRS screen reads its permissions from what is set here, User Maintenance is the linchpin of the application's security model — it is referenced as the "access model" source by [[Communicable Disease Maintenance Screen Specifications]] and [[Create Outbreak Investigation Screen Specifications]].

It is one of the three "Foundation" maintenance functions in the design deck (slides 6–8): this **User Maintenance** screen, Location / Facility Maintenance, and [[Communicable Disease Maintenance Screen Specifications|Communicable Disease Maintenance]]. The **design guideline** is to align the OMRS access model with the Connect Care (Epic) communicable-disease roles to the degree practical, so that a user's outbreak responsibilities mirror their clinical responsibilities and provisioning stays consistent across the two applications. `CDOM User Roles.pdf` is the current-state reference the model maps from.
...
The PowerPoint version of this wireframe is *OMRS User Maintenance* (slide 6) in [Alberta Outbreak Management Reporting Application (OMRA) Design.pptx](https://albertahealthservices.sharepoint.com/teams/M365TCCNotifiableDiseaseProjects/Shared%20Documents/General/Solution%20Design%20Documents/Outbreak%20Management%20Application/Alberta%20Outbreak%20Management%20System%20Design.pptx).
...
1. **Scenario Conditions** — What scenarios have been defined that may affect the specifications.
   ...
   1. *Provisioning a new user* — A **Team Lead or Department Manager** looks up a person, assigns exactly **one functional role**, and attaches the **department** scoping attribute, a professional designation, and any maintenance flags. The current CD/OM application bundled "what a user can do", "what data a user can see" and "who the user is" under the single word *role*; OMRS separates these so the same person no longer needs several "roles" at once.
...
3. **User Experience Considerations** — What (if any) UX options might we have to consider as part of the build.
   ...
   1. This function is part of the OMRS **Foundation** maintenance set; reuse CDOM UX where appropriate (deck slide 5) and keep it visually consistent with [[Communicable Disease Maintenance Screen Specifications|Communicable Disease Maintenance]] and Location / Facility Maintenance.
...
4. **Data Inputs and Outputs** — What are the data elements involved for the build object. What test data is needed to support functional testing.
   ...
   2. *Outputs* — The user / access record `{ role, department[], designation, maintenanceFlags[] }` consumed by every OMRS screen for department-level data scoping (edit own department, read all others) and action authorization; attribute-change and privileged-action entries written to `AuditLog`.
...
> Retained from the original Role-Based Access Specification so this screen spec remains the canonical access-model reference cited by the other OMRS specs. The screen sections above implement this model; the sections below define it.
...
OMRS separates these concerns into two layers:
...
| OMRS role | Connect Care alignment |
|---|---|
