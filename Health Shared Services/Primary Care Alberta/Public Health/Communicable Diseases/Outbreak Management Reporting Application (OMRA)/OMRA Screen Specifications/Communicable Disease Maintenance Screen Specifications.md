...
description: Build specification for the OMRA Communicable Disease (Disease Dictionary) Maintenance screen, mapping the OMRA Disease Maintenance wireframe to the OMRA database model.
tags:
...
- **Wireframe** — *OMRA Disease Maintenance*, slide 8 of [Alberta Outbreak Management Reporting Application (OMRA) Design.pptx](https://albertahealthservices.sharepoint.com/teams/M365TCCNotifiableDiseaseProjects/Shared%20Documents/General/Solution%20Design%20Documents/Outbreak%20Management%20Application/Alberta%20Outbreak%20Management%20System%20Design.pptx).
- **Data model** — [[OMRA Database ERD]] (v2.5), principally the `InfectiousDisease` (incl. `isProvisional`), `OutbreakDefinition`, `Symptom` / `DiseaseSymptom`, `Organism` / `DiseaseOrganism`, `Vaccine` / `DiseaseVaccine`, `DiagnosticTest` / `DiseaseTest`, `DiseaseCaseDefinitionRule`, and `VersionStatus` tables.
...
The PowerPoint version of this wireframe is *OMRA Disease Maintenance* (slide 8) in [Alberta Outbreak Management Reporting Application (OMRA) Design.pptx](https://albertahealthservices.sharepoint.com/teams/M365TCCNotifiableDiseaseProjects/Shared%20Documents/General/Solution%20Design%20Documents/Outbreak%20Management%20Application/Alberta%20Outbreak%20Management%20System%20Design.pptx).
...
1. **Scenario Conditions** — What scenarios have been defined that may affect the specifications.
   ...
   3. *Disease attributes drive downstream behaviour* — Reporting timing, case classification (Confirmed / Probable), and outbreak thresholds are all consumed elsewhere in OMRA, so edits here ripple into live investigations. Changes are **versioned, not overwritten**.
...
8. **Data Inputs and Outputs** — What are the data elements involved for the build object. What test data is needed to support functional testing.
   ...
   12. **ERD support (resolved in v2.1)** — the four model elements the wireframe needs are now in [[OMRA Database ERD]] (v2.1): the `Vaccine` + `DiseaseVaccine` tables (Associated Vaccine), the `DiagnosticTest` + `DiseaseTest` tables (Lab Procedures), the `DiseaseCaseDefinitionRule` table (structured Confirmed/Probable Episode Conditions, replacing free-text `caseDefinitionText`), and new `OutbreakDefinition` fields `minConfirmedCaseCount`, `minProbableCaseCount`, `absenteeismThresholdPct` and `closureCriteriaText` (setting-specific thresholds). The field table below maps to these tables.
...
- **Last Update**
  - **June 17, 2026 (Alec Blair)** — Initial draft. Populated the specification from the *OMRA Disease Maintenance* wireframe (slide 8 of the OMRA Design deck), mapped each field to the [[OMRA Database ERD]], and aligned the access model to [[User Maintenance Screen Specifications]]. Flagged four ERD gaps for resolution before build: Associated Vaccine dimension, Lab Procedure / Diagnostic Test dimension, structured Episode-Condition rule table (replacing free-text case definitions), and setting-specific closure thresholds (e.g., school-absenteeism %).
  - **June 17, 2026 (Alec Blair)** — Updated the field mapping to the resolved [[OMRA Database ERD]] v2.1: Lab Procedures → `DiagnosticTest`/`DiseaseTest`; Associated Vaccine → `Vaccine`/`DiseaseVaccine`; Confirmed/Probable Episode Conditions → `DiseaseCaseDefinitionRule`; Minimum Case Definition Rule and Closure Criteria → new `OutbreakDefinition` fields (`minConfirmedCaseCount`, `minProbableCaseCount`, `absenteeismThresholdPct`, `closureCriteriaText`). No remaining ERD gaps.
  - **June 25, 2026 (Alec Blair)** — Documented the **Add Disease** minimum-viable quick-add (Disease Name + Reporting Timeline mandatory; “Add to Dictionary” writes a new Active version; remaining attributes completed in the editor) to align the specification with the OMRA prototype. Added Functional Behaviour 2.5.
  - **June 25, 2026 (Alec Blair)** — Synced to [[OMRA Database ERD]] v2.5: the quick-add now sets `InfectiousDisease.isProvisional = true` so the entry is immediately selectable but flagged incomplete/pending curation. Updated FB 2.5 and added a `Provisional? (quick-add)` field row.
