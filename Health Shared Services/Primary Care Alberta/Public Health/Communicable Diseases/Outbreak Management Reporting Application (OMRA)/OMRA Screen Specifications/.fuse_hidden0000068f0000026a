...
description: Build specification for the OMRS Create Outbreak Investigation screen, mapping the Outbreak Summary wireframe to the OMRS database model and establishing the minimum-viable record that anchors the rest of an investigation.
tags:
...
This screen creates the **outbreak investigation record** that the rest of the Outbreak Management Reporting System (OMRS) hangs off — the line list, aggregate and AORF reporting, the contact list, and the outbreak team all reference the `Outbreak` row created here. The design intent is a **minimum-viable open**: capture just enough to start an investigation, then enrich the record over time as the situation is understood.

It is the entry point to the OMRS investigation set and sits alongside [[Contact Identification Screen Specifications|Contact Identification]] (where the contacts exposed within an outbreak are worked) and the Foundation maintenance screens — [[User Maintenance Screen Specifications|User Maintenance]], Location / Facility Maintenance, and [[Communicable Disease Maintenance Screen Specifications|Communicable Disease Maintenance]] — that supply its lookups. The Disease lookup is supplied by the disease dictionary, including dictionary entries created through the [[Communicable Disease Maintenance Screen Specifications|Disease Maintenance]] **Add Disease** quick-add (which are immediately selectable but flagged `isProvisional` in [[OMRS Database ERD]] v2.5).

Because TB and (occasionally) STI investigations use the outbreak framework even when they do not meet the formal outbreak case definition, and because the Outbreak ID is expected to serve as the lab-requisition / Exposure Investigation identifier, this screen must support **large-exposure** and **region** investigations, not only facility outbreaks. Connect Care remains the **system of record for individual cases** (see [[CLAUDE-OMRS]] data-ownership boundaries); OMRS owns the outbreak coordination record.
...
- **Data model** — [[OMRS Database ERD]] (v2.5), principally the `Outbreak` entity (Section 1) and its `OutbreakType`, `OutbreakOrganism`, `OutbreakLifecycleStatus`, `OutbreakTeamMember`, `Facility` / `Building` / `Department`, `Zone`, `OutbreakSetting` and `InfectiousDisease` references.
...
3. **User Experience Considerations** — What (if any) UX options might we have to consider as part of the build.
   ...
   1. This function is part of the overall OMRS Outbreak Management set; keep it visually consistent with [[Contact Identification Screen Specifications|Contact Identification]] and the other investigation screens.
...
|---|---|---|---|---|---|---|
| Outbreak ID | Alpha Numeric | `Outbreak.outbreakID` | Auto-generated | None – auto | Yes | Immutable; expected to also serve as the EI / lab-requisition number (deferred, [[OMRS Database ERD]] note 10). |
...
- **Access Specifications** — Creating an outbreak investigation is investigator activity within the OMRS access model ([[User Maintenance Screen Specifications]]). Confirm whether outbreak creation is restricted to specific functional roles (e.g., Investigator and above) and scoped by team/zone/disease group, or open to all intake users.
...
- **Last Update**
  ...
  - **June 25, 2026 (Alec Blair)** — Brought the specification up to the OMRS screen-specification standard used by [[Communicable Disease Maintenance Screen Specifications]] and [[Contact Identification Screen Specifications]]: added OKF frontmatter; rewrote the context and wireframe sections as prose with linked source material; expanded the seven Acceptance Criteria sections; and rebuilt the field table mapped to [[OMRS Database ERD]] v2.5, flagging four ERD gaps inline for resolution before build — no direct `Outbreak.infectiousDiseaseID` (Disease), no `Outbreak.outbreakSettingID` (Setting), no `Outbreak.zoneID`/region columns (Zone/Region), and no outbreak-level department linkage (Department).
