# 04 — Expenses & Spending Plan

*Input file. The target number that decides whether either retirement date works. Pull from [[Current Monthly Budget]] where possible.*

---

## Current annual spending (pre-retirement baseline)

| Category | Monthly | Annual | Notes |
|---|---|---|---|
| Housing (mortgage/rent, tax, utilities, maintenance) |  |  |  |
| Food & household |  |  |  |
| Transportation |  |  |  |
| Insurance (life, home, auto, health) |  |  |  |
| Healthcare / dental / prescriptions |  |  |  |
| Travel & leisure |  |  |  |
| Subscriptions & memberships |  |  |  |
| Family / gifts / support |  |  |  |
| Discretionary / other |  |  |  |
| **Total** |  |  |  |

## Projected retirement spending

Retirement spending usually isn't flat. Model three phases:

| Phase | Years (approx) | Annual spend | Reasoning |
|---|---|---|---|
| **Go-go** (active, more travel) | first ~10 yrs |  | Often *higher* than working-life baseline |
| **Slow-go** (less travel) | next ~10 yrs |  | Typically declines |
| **No-go** (later years, more care) | final years |  | Travel down, healthcare up |

### Spending changes at retirement
What disappears or appears once you stop working:

- Removed: commuting, work wardrobe, employment-related costs, EI/CPP contributions, RRSP saving, mortgage (if paid off — see file 05)
- Added: private health/dental premiums (file 09), more travel/hobbies, possibly higher home/auto use

## Must-have vs. nice-to-have floor

- **Essential / non-negotiable annual spend (the floor):** `$______`
- **Comfortable target annual spend:** `$______`
- **Aspirational (everything you'd want):** `$______`

> The pension's job is ideally to cover the **floor**. Savings cover the gap to **comfortable/aspirational**. File 06 tests this against both retirement dates.

---

*Tie-in: the floor here vs. the guaranteed pension income in file 02 is the heart of the worst-case test.*
